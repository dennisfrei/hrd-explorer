...
export function drawHRD(hx, R, state) {
  ...
  hx.fillStyle = '#050508'; hx.fillRect(0, 0, W, H);
  ...
  if (state.selStar) drawHL(hx, R, state.selStar, 'rgba(255,255,255,0.65)');
  if (state.cmpStar) drawHL(hx, R, state.cmpStar, 'rgba(120,200,255,0.55)');
}

function drawColorBG(hx, R) {
  for (let px = 0; px < R.w; px += 3) {
    const [r2, g, b] = tRGB(xteff(R.x + px));
    hx.fillStyle = `rgba(${r2},${g},${b},0.06)`;
    hx.fillRect(R.x + px, R.y, 3, R.h);
  }
}

function drawGrid(hx, R) {
  hx.strokeStyle = 'rgba(140,160,220,0.055)'; hx.lineWidth = 1;
...
function drawRiso(hx, R) {
  ...
  radii.forEach((Rv, i) => {
    ...
    hx.strokeStyle = 'rgba(140,160,220,0.16)'; hx.lineWidth = 0.8; hx.setLineDash([3, 5]);
    ...
    if (lL >= LMIN && lL <= LMAX) {
      ...
      if (lx > R.x + 5 && lx < R.x + R.w - 10) {
        ...
        hx.fillStyle = 'rgba(140,160,220,0.36)'; hx.font = '9px JetBrains Mono,monospace';
...
function drawZAMSline(hx, R) {
  hx.strokeStyle = 'rgba(180,200,255,0.28)'; hx.lineWidth = 1.8; hx.setLineDash([]);
  hx.beginPath(); let first = true;
  ...
  hx.stroke();
  const [mt, ml] = ZAMS[7];
  hx.fillStyle = 'rgba(180,200,255,0.32)'; hx.font = '9px Space Grotesk,sans-serif';
...
function drawRegions(hx, R) {
  ...
  hx.font = '9px Space Grotesk,sans-serif'; hx.fillStyle = 'rgba(140,160,220,0.22)';
  regs.forEach(({ l, t, L }) => {
    const x = tx(t), y = ly(L);
...
function drawAxes(hx, R) {
  hx.strokeStyle = 'rgba(140,160,220,0.28)'; hx.lineWidth = 1; hx.strokeRect(R.x, R.y, R.w, R.h);
  hx.font = '10px JetBrains Mono,monospace'; hx.fillStyle = 'rgba(140,160,220,0.52)';
  hx.textAlign = 'right';
  ...
  hx.font = '10px Space Grotesk,sans-serif'; hx.fillStyle = 'rgba(140,160,220,0.42)';
  hx.textAlign = 'center'; hx.fillText('log(L / L☉)', 0, 0); hx.restore();
  hx.textAlign = 'center'; hx.fillStyle = 'rgba(140,160,220,0.35)'; hx.font = '9px Space Grotesk,sans-serif';
...
function drawDots(hx, R, state) {
  ...
  STARS.forEach(s => {
    ...
    if (isSel || isCmp) {
      hx.strokeStyle = isCmp ? 'rgba(120,200,255,0.8)' : 'rgba(255,255,255,0.82)';
      hx.lineWidth = 1.5; hx.beginPath(); hx.arc(x, y, dot + 2.5, 0, Math.PI * 2); hx.stroke();
      hx.fillStyle = 'rgba(255,255,255,0.72)'; hx.font = '10px Space Grotesk,sans-serif';
...
function drawCross(hx, R, pos) {
  ...
  hx.strokeStyle = 'rgba(140,160,220,0.16)'; hx.lineWidth = 1; hx.setLineDash([2, 4]);
